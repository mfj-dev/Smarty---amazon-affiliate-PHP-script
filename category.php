...
$this_category_url = $_GET['url']; // This is url of your category that is used for selectin data from MYSQL database
include 'includes/category-product-query.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <!-- HEADER OF A CATEGORY PAGE WITH LOGO AND TITLE TAG SPECIFIC FOR CATEGORY PAGE -------- START -->
    <?php
    include 'includes/category-header.php';
    ?>
    <!-- HEADER OF A CATEGORY PAGE WITH LOGO AND TITLE TAG SPECIFIC FOR CATEGORY PAGE  -------- END -->
    ...
    $best_product_query = "SELECT * FROM products WHERE product_category_id=$product_category_id ORDER by product_avg_rating DESC LIMIT 1";
...
<li><a href="'.$website_frontend_navigation_link2_url.''.$sufixwithpar.''. $product_category_url.'">'.$product_category.'</a></li>
</ul>
<div class="clearfix"></div>
<div class="two-third-column">';
            }
            ...
            $this_category_query = "SELECT * FROM product_categories WHERE product_category_url='$this_category_url'";
            foreach ($connread->query($this_category_query) as $row) {
                $this_category_h1 = $row['product_category_h1'];
                $best_product_query = "SELECT * FROM products WHERE product_category_id=$product_category_id ORDER by product_avg_rating DESC LIMIT 1";
                foreach ($connread->query($best_product_query) as $row) {
                    ?>
<h1><strong><?php echo $this_category_h1; ?></strong></h1>
...
<?php } ?>
            ...
            <?php if ($total_pages > 1) {
                include 'includes/category-pagination.php';
            }
            ?>
        </div>
    ...
    <!-- MAIN CONTENT OF THE CATEGORY PAGE -------- END -->
    <div class="clearfix"></div>
    <br/>
    <!-- FOOTER AREA -------- START -->
    <?php 
