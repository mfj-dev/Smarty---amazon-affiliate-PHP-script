<?php
/**
 * THIS IS PART FOR SELECTING PRODUCT BRANDS FROM DATABASE DEPENDING OF URL PARAMETERS  ---- START
 */
$product_brands_query = "SELECT * FROM product_brands WHERE product_brand_url='$this_brand_url'";
foreach ($connread->query($product_brands_query) as $row) {
    ...
    /**
     * THIS IS PART FOR SELECTING PRODUCT BRANDS FROM DATABASE DEPENDING OF URL PARAMETERS  ---- END
     */
    ?>
    <title><?php echo $product_brand_title_tag; ?></title>
    <meta name="description" content='<?php echo $product_brand_meta_tag; ?>'>
<?php } ?>
<?php
/**
...
<?php
    if (isset($_GET['page']) && $page == $total_pages) {
        ?>
        <link rel="prev"
              href="<?php echo $website_frontend_navigation_link3_url; ?>.php?url=<?php echo $this_brand_url; ?>&&page=<?php echo $pageminus; ?>">
    <?php } ?>
    <?php
    if (isset($_GET['page']) && $page == 1) {
        ?>
        <link rel="next"
              href="<?php echo $website_frontend_navigation_link3_url; ?>.php?url=<?php echo $this_brand_url; ?>&&page=<?php echo $pageplus; ?>">
    <?php } ?>
