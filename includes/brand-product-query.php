...
foreach ($connread->query($this_brand_query) as $row) {
    ...
    $products_count_query = "SELECT COUNT(*) as num FROM products WHERE product_brand_id=$product_brand_id";
...
foreach ($connread->query($this_brand_query) as $row) {
    $product_brand_id = $row['product_brand_id'];
    $latest_product_query = "SELECT * FROM products WHERE product_brand_id=$product_brand_id  ORDER by product_id DESC LIMIT $start, $limit";
}